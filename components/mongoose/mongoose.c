...
void cs_log_print_prefix(const char *func) {
  if (cs_log_file == NULL) cs_log_file = stdout;
...
double cs_time(void) {
  double now;
#ifndef _WIN32
  now = xTaskGetTickCount() * portTICK_PERIOD_MS / 1000.0;
#else
...
static void mg_http_construct_etag(char *buf, size_t buf_len,
                                   const cs_stat_t *st) {
  snprintf(buf, buf_len, "\"%lx.%d\"", (unsigned long) st->st_mtime,
           (int) st->st_size);
}
